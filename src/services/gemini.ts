...
const getVocabularyEntry = (english: string) =>
  vocabulary.find((entry) => normalizeText(entry.english) === normalizeText(english));

const toPracticeSentence = (polish: string, englishVariants: string[]): PracticeSentence => ({
  polish,
  english: englishVariants[0],
  acceptableTranslations: englishVariants,
});

const buildVerbFallbacks = (word: string, polishLabel: string): PracticeSentence[] => {
  const normalizedWord = normalizeText(word);

  if (normalizedWord === 'be') {
    return [
      toPracticeSentence('Jestem dzisiaj spokojny.', ['I am calm today.', "I'm calm today."]),
      toPracticeSentence('Oni są teraz w domu.', ['They are at home now.', "They're at home now."]),
      toPracticeSentence('Czy ona jest gotowa?', ['Is she ready?']),
      toPracticeSentence('To jest bardzo ważne.', ['This is very important.']),
      toPracticeSentence('Nie jesteśmy spóźnieni.', ['We are not late.', "We aren't late."]),
    ];
  }

  if (normalizedWord === 'have') {
    return [
      toPracticeSentence('Mam dziś dużo pracy.', ['I have a lot of work today.']),
      toPracticeSentence('Ona ma dobry pomysł.', ['She has a good idea.']),
      toPracticeSentence('Czy masz chwilę?', ['Do you have a minute?']),
      toPracticeSentence('Nie mamy teraz czasu.', ["We do not have time now.", "We don't have time now."]),
  ...
  if (normalizedWord === 'do') {
    return [
      ...
      toPracticeSentence('Ona nie robi tego często.', ["She does not do it often.", "She doesn't do it often."]),
      toPracticeSentence('Zróbmy to teraz.', ['Let us do it now.', "Let's do it now."]),
  ...
  return [
    ...
    toPracticeSentence(`Nie chcę ${polishLabel} dzisiaj.`, [`I do not want to ${word} today.`, `I don't want to ${word} today.`]),
    toPracticeSentence(`Czy potrafisz ${polishLabel} szybko?`, [`Can you ${word} quickly?`]),
    toPracticeSentence(`Oni muszą ${polishLabel} razem.`, [`They have to ${word} together.`]),
    toPracticeSentence(`Ona lubi ${polishLabel} rano.`, [`She likes to ${word} in the morning.`]),
  ];
};

const buildPhraseFallbacks = (word: string, polishLabel: string): PracticeSentence[] => [
  toPracticeSentence(`W takiej sytuacji mówię: "${polishLabel}".`, [`In this situation I say: "${word}."`, `In this situation, I say: "${word}."`]),
  toPracticeSentence(`Na koniec rozmowy możesz powiedzieć: "${polishLabel}".`, [`At the end of the conversation, you can say: "${word}."`]),
  toPracticeSentence(`To krótkie wyrażenie znaczy: "${polishLabel}".`, [`This short phrase means: "${word}."`]),
  toPracticeSentence(`Często słyszysz zdanie: "${polishLabel}".`, [`You often hear the phrase: "${word}."`]),
  toPracticeSentence(`Najprostsza odpowiedź brzmi: "${polishLabel}".`, [`The simplest answer is: "${word}."`]),
];

const buildNounFallbacks = (word: string, polishLabel: string): PracticeSentence[] => [
  toPracticeSentence(`To jest mój ${polishLabel}.`, [`This is my ${word}.`]),
  toPracticeSentence(`Potrzebuję tego ${polishLabel} teraz.`, [`I need this ${word} now.`]),
  toPracticeSentence(`Ten ${polishLabel} jest bardzo ważny.`, [`This ${word} is very important.`]),
  toPracticeSentence(`Mamy więcej niż jeden ${polishLabel}.`, [`We have more than one ${word}.`]),
  toPracticeSentence(`Czy widzisz ten ${polishLabel}?`, [`Can you see this ${word}?`]),
];

const buildAdjectiveFallbacks = (word: string, polishLabel: string): PracticeSentence[] => [
  toPracticeSentence(`To jest bardzo ${polishLabel}.`, [`It is very ${word}.`]),
  toPracticeSentence(`Ten budynek wydaje się ${polishLabel}.`, [`This building seems ${word}.`]),
  toPracticeSentence(`Dzisiaj czuję się bardziej ${polishLabel}.`, [`I feel more ${word} today.`]),
  toPracticeSentence(`Czy to wygląda ${polishLabel}?`, [`Does it look ${word}?`]),
  toPracticeSentence(`Ich plan jest naprawdę ${polishLabel}.`, [`Their plan is really ${word}.`]),
];

const buildFallbackSentences = (word: string, type: string): PracticeSentence[] => {
...
export async function generateSentences(word: string, type: string): Promise<PracticeSentence[]> {
  const normalizedWord = normalizeText(word);
  ...
  return combined.filter((sentence) => {
...
export async function explainError(polish: string, expectedEnglish: string, userEnglish: string): Promise<string> {
  ...
  const notes: string[] = [`Porównaj odpowiedź z wzorcem dla zdania: "${polish}".`];
  ...
  if (extraTokens.length > 0) {
    notes.push(`Masz też dodatkowe słowa: ${extraTokens.slice(0, 4).join(', ')}.`);
  }
...
export async function evaluateTranslation(polish: string, userEnglish: string, context: string): Promise<EvaluationResult> {
  const isQuestion = polish.trim().endsWith('?');
  const normalizedInput = normalizeText(userEnglish);
  const looksQuestion = normalizedInput.startsWith('do ') || normalizedInput.startsWith('does ') || normalizedInput.startsWith('did ') || normalizedInput.startsWith('is ') || normalizedInput.startsWith('are ') || normalizedInput.startsWith('was ') || normalizedInput.startsWith('were ') || normalizedInput.startsWith('have ') || normalizedInput.startsWith('has ') || normalizedInput.startsWith('had ') || normalizedInput.startsWith('will ') || normalizedInput.startsWith('can ');
  ...
  return {
    isCorrect: true,
    feedback: 'Tłumaczenie wygląda sensownie. Porównaj je jeszcze z odpowiedzią wzorcową, zwłaszcza pod kątem operatorów i szyku.',
  };
}
...
export async function translateWord(word: string, context: string): Promise<string> {
  ...
  const partialMatch = vocabulary.find((entry) =>
    entry.polish
      .split('/')
      .map((variant) => normalizeText(variant))
      .some((variant) => variant.includes(normalizedWord) || normalizedWord.includes(variant))
  );
...
export async function getHint(polish: string, context: string): Promise<string> {

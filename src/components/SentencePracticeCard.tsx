...
import { CheckCircle2, Eye, Lightbulb, Loader2, MessageSquareText, XCircle, HelpCircle } from 'lucide-react';
import { PracticeSentence, explainError, getHint, translateWord, EvaluationResult } from '../services/gemini';
...
export function SentencePracticeCard({ sentence, index, context }: SentencePracticeCardProps) {
  const [userInput, setUserInput] = useState('');
  const [status, setStatus] = useState<'idle' | 'evaluating' | 'evaluated' | 'revealed'>('idle');
  ...
  const handleCheck = () => {
    ...
    const isCorrect = acceptable.some(t => normalizeText(t) === normalizedInput);
    ...
    setTimeout(() => {
      if (isCorrect) {
        ...
        setEvaluation({
          isCorrect: false,
          feedback: 'Niestety, to nie jest poprawne tłumaczenie. Spróbuj jeszcze raz lub sprawdź odpowiedź.'
        });
      }
  ...
  const handleExplainError = async () => {
    ...
    const explanation = await explainError(sentence.polish, sentence.english, userInput);
  ...
  return (
    ...
    >
      <div className="mb-4">
        <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1 block">Zdanie {index + 1}</span>
        ...
        {translatedWord && (
          ...
          >
            ...
            {isTranslating ? (
              ...
              <span className="font-bold text-indigo-600 dark:text-indigo-400">{translatedWord.translation}</span>
            )}
            <button 
      ...
      <div className="space-y-4">
        ...
        {status !== 'revealed' && (
          <div className="flex flex-wrap gap-3 items-center justify-between">
            ...
            <div className="flex gap-3">
              ...
              >
                {status === 'evaluating' ? <Loader2 className="w-4 h-4 animate-spin" /> : <MessageSquareText className="w-4 h-4" />}
                Sprawdź
              </button>
            </div>
          </div>
          ...
          >
            <div className="flex gap-3 items-start">
              ...
              <div className="flex-1">
                <p className={evaluation.isCorrect ? 'text-emerald-900 dark:text-emerald-400' : 'text-rose-900 dark:text-rose-400'}>
                  {evaluation.feedback}
                </p>
                {evaluation.isCorrect && (
                  <div className="mt-3 pt-3 border-t border-emerald-200/50 dark:border-emerald-800/50">
                    <span className="text-xs font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-wider mb-1 block">Wzorcowa odpowiedź</span>
                    <p className="text-emerald-800 dark:text-emerald-300 font-medium">{sentence.english}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {!evaluation.isCorrect && !aiExplanation && (
              <div className="mt-2 flex justify-end">
                ...
                >
                  {isExplaining ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <HelpCircle className="w-3.5 h-3.5" />}
                  Wyjaśnij błąd
                </button>
              </div>
            )}
            ...
            {aiExplanation && (
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-1 block">Wyjaśnienie</span>
        ...
        {status === 'revealed' && (
          ...
          >
            ...
            <div>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 block">Poprawna odpowiedź</span>
              <p className="text-slate-900 dark:text-white font-medium">{sentence.english}</p>
            </div>
          </motion.div>
        )}
      </div>

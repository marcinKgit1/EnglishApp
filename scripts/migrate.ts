  ...
  for (const sentence of practiceSentences) {
    ...
    for (const level of learningPath) {
      const branch = level.branches.find(b => {
        ...
        if (topic === 'present perfect' && title.includes('present perfect simple')) return true;
        if (topic === 'be going to' && title.includes('going to')) return true;
        if (topic === 'first conditional' && title.includes('1st conditional')) return true;
        if (topic === 'second conditional' && title.includes('2nd')) return true;
        if (topic === 'past perfect' && title.includes('past perfect simple')) return true;
        return false;
      });
      if (branch) {
        branchId = branch.id;
